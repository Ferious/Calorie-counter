...
public class ActivityRecord extends Menu {
    ...
    public void handle(String option) {
        try {
            if (state == ActivityState.START) {
                switch (option){
                    case "S" : case "s": setState(ActivityState.PROGRESS); start(); break;
                ...
                switch (option){
                    case "P" : case "p": setState(ActivityState.PAUSE); stop(); break;
                    case "E" : case "e": setState(ActivityState.END); stop(); enterName();  break;
                    default  : System.err.println(String.format("%s is a unknown option", option)); break;
                }
            }
            else if (state == ActivityState.PAUSE) {
                switch (option){
                    case "R": case "r": setState(ActivityState.PROGRESS); start(); break;
                    case "E": case "e": setState(ActivityState.END); enterName(); break;
                    default  : System.err.println(String.format("%s is a unknown option", option)); break;
                }
            }
            else if (state == ActivityState.END) {
                switch (option){
                    case "1" : activity.setDifficulty(ActivityDifficulty.EASY); calculateCalories(); save(); break;
                    case "2" : activity.setDifficulty(ActivityDifficulty.MODERATE); calculateCalories(); save(); break;
                    case "3" : activity.setDifficulty(ActivityDifficulty.HARD); calculateCalories(); save(); break;
                    default  : System.err.println(String.format("%s is a unknown option", option)); break;
                }
            }
